...
int instruction_flag = 1484, att_type = 0; // ######## look for this string ########
uint32_t random_16b_insn = 0x5f1a;   // ******** look for this string ********
target_ulong rand_pc = 0x1;         // ^^^^^^^^^^ look for this string ^^^^^^^^^
...
static void disas_arm_insn(CPUARMState * env, DisasContext *s)
{
    ...
    instruction_count ++ ;
    printf("instruction_count = %d/n", instruction_count);
    insn = arm_ldl_code(env, s->pc, s->bswap_code);
    printf("insn arm: 0x%08X -----------------------------------\n", insn);
    s->pc += 4;
...
static int disas_thumb2_insn(CPUARMState *env, DisasContext *s, uint16_t insn_hw1)
{
    ...
    printf("insn thumb 2: 0x%08X *********\n", insn);
    if(instruction_count == instruction_flag  && att_type <= 3){
        if(att_type == 0){ //flip
            insn =~insn;

        }
        if(att_type == 1){ //all zero
            insn = 0x0;
        }
        if(att_type == 2){ //all one
            insn = 0xFFFFFFFF;
        }
        if(att_type == 3){ //randomize 16 first bits
            insn = (insn & 0xFFFF0000) + random_16b_insn;
        }

    }
        s->pc += 2;
      if(instruction_count == instruction_flag  && att_type == 4){ //random pc addition
        s->pc+=rand_pc;
    }
